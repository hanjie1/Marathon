...
void eff_forward()
{
	 ...
	 infile1.open(He_file);
         ...
         //     cout<<run_number<<endl;
              Check=count_forward(run_number,kin,He_gele,He_notrack); 
              if(Check==-1){
         ...
         while(line.ReadToken(infile2) ){
              run_number=line.Atoi();
              Check=count_forward(run_number,kin,H3_gele,H3_notrack); 
              if(Check==-1){
                 cout<<run_number<<" rootfile couldn't find"<<endl;
                 continue;
              }
   ...
   c1->Print("H3_He_all.png");

   TGraphErrors *hHe_1=new TGraphErrors(10,xbj,He_eff,xbj_err,He_eff_err);
   TGraphErrors *hH3_1=new TGraphErrors(10,xbj,H3_eff,xbj_err,H3_eff_err);

   TCanvas *c2=new TCanvas();
   TMultiGraph *mg1=new TMultiGraph();
   mg1->Add(hHe_1);
   hHe_1->SetMarkerColor(1);
   hHe_1->SetMarkerStyle(8);
   mg1->Add(hH3_1);
   hH3_1->SetMarkerColor(2);
   hH3_1->SetMarkerStyle(8);
   mg1->Draw("AP");

   auto leg1 = new TLegend(0.1,0.75,0.25,0.90);
   leg1->AddEntry(hHe,"He3","P");
   leg1->AddEntry(hH3,"Tritium","P");
   leg1->Draw();

   c2->Print("H3_He_LHRS.png");

   ofstream myfile;
   myfile.open("H3_He_kin16.txt");
   for(int jj=0;jj<11;jj++){
