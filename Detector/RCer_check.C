...
void Check(int nrun)
{
    ...
    for(int ii=0;ii<10;ii++){
	 ...
	 T->Draw(Form("R.cer.a_p[%d]>>hcer_p[%d]",ii,ii));
     ...
     for(int ii=0;ii<10;ii++){
         ...
         hcer_c[ii]->Fit(g1,"NQ","",mean1-100,mean1+100);
         ...
         if(mean2<200||mean2>500){
                ...
                peak_wid[ii]=sigma1;
                flag=1;
                hcer_c[ii]->Fit(g1,"Q","",200,500);
                gPad->SetLogy();
                continue;
         }

         hcer_c[ii]->Fit(g1,"Q","",mean2-100,mean2+100);
         ...
         if(mean3<200||mean3>500){
                ...
                peak_wid[ii]=sigma2;
                flag=1;
                hcer_c[ii]->Fit(g1,"Q","",mean1-100,mean1+100);
                gPad->SetLogy();
                continue;
         }
    ...
    outfile1.open("OUT/Rcer_ped.dat",fstream::app);
    outfile2.open("OUT/Rcer_ped_wid.dat",fstream::app);
    outfile3.open("OUT/Rcer_peak.dat",fstream::app);
    outfile4.open("OUT/Rcer_peak_wid.dat",fstream::app);
    outfile1<<nrun<<"  ";
    outfile2<<nrun<<"  ";
    outfile3<<nrun<<"  ";
    outfile4<<nrun<<"  ";
    ...
    outfile3.close();
    outfile4.close();
    c1->Print(Form("PDF/Rcer_%d.pdf[",nrun));
    c1->Print(Form("PDF/Rcer_%d.pdf",nrun));
    c2->Print(Form("PDF/Rcer_%d.pdf",nrun));
    c2->Print(Form("PDF/Rcer_%d.pdf]",nrun));
