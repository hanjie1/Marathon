...
void Check(int nrun)
{
    ...
    for(int ii=0;ii<16;ii++){
        delete gROOT->FindObject(Form("hs2_lp[%d]",ii));
     ...
     for(int ii=0;ii<16;ii++){
         c1->cd(ii+1);
         hs2_lp[ii]=new TH1F(Form("hs2_lp[%d]",ii),"s2 la_p",300,-200,1000);
	 T->Draw(Form("R.s2.la_p[%d]>>hs2_lp[%d]",ii,ii));
     ...
     for(int ii=0;ii<16;ii++){
         c2->cd(ii+1);
         hs2_lc[ii]=new TH1F(Form("hs2_lc[%d]",ii),"s2L a_c",300,-200,800);
         T->Draw(Form("R.s2.la_c[%d]>>hs2_lc[%d]",ii,ii));
    ...
    outfile1.open("OUT/Rs2L_ped.dat",fstream::app);
    outfile2.open("OUT/Rs2L_ped_wid.dat",fstream::app);
    outfile3.open("OUT/Rs2L_peak.dat",fstream::app);
    outfile4.open("OUT/Rs2L_peak_wid.dat",fstream::app);
    outfile1<<nrun<<"  ";
    outfile2<<nrun<<"  ";
    outfile3<<nrun<<"  ";
    outfile4<<nrun<<"  ";
    ...
    outfile3.close();
    outfile4.close();
    c1->Print(Form("PDF/Rs2L_%d.pdf[",nrun));
    c1->Print(Form("PDF/Rs2L_%d.pdf",nrun));
    c2->Print(Form("PDF/Rs2L_%d.pdf",nrun));
    c2->Print(Form("PDF/Rs2L_%d.pdf]",nrun));
