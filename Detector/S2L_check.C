...
void Check(int nrun)
{
    ...
    for(int ii=0;ii<16;ii++){
	 ...
	 T->Draw(Form("L.s2.la_p[%d]>>hs2_lp[%d]",ii,ii));
    ...
    outfile1.open("OUT/Ls2L_ped.dat",fstream::app);
    outfile2.open("OUT/Ls2L_ped_wid.dat",fstream::app);
    outfile3.open("OUT/Ls2L_peak.dat",fstream::app);
    outfile4.open("OUT/Ls2L_peak_wid.dat",fstream::app);
    outfile1<<nrun<<"  ";
    outfile2<<nrun<<"  ";
    outfile3<<nrun<<"  ";
    outfile4<<nrun<<"  ";
    ...
    outfile3.close();
    outfile4.close();
    c1->Print(Form("PDF/s2L_%d.pdf[",nrun));
    c1->Print(Form("PDF/s2L_%d.pdf",nrun));
    c2->Print(Form("PDF/s2L_%d.pdf",nrun));
    c2->Print(Form("PDF/s2L_%d.pdf]",nrun));
