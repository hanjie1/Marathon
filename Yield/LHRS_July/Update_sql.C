...
int Update(int run_number=0,TString target="NULL",int kin=0){
  ...
  int ncurrent=CheckCurrent(run_number,kin);
  ...
  if(insert){
      query=Form("insert %s (run_number,target,kinematic,livetime,charge,target_thickness,current) values(%d,'%s',%d,%lf,%lf,%lf,%d);",table.Data(),run_number,target.Data(),kin,livetime.value,charge,target_thickness,ncurrent);
...
//      query=Form("update %s set target='%s',kinematic=%d,livetime=%lf,charge=%lf,target_thickness=%lf,current=%d where run_number=%d",table.Data(),target.Data(),kin,livetime,charge,target_thickness,ncurrent,run_number);
      query=Form("update %s set livetime=%lf,LT_err=%lf where run_number=%d",table.Data(),livetime.value,livetime.err,run_number);
      result=Server->Query(query.Data());
      if(!result){
