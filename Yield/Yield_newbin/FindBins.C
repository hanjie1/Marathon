...
void FindBins()
{
   ...
   Double_t x_min[11]={0.0},x_max[11]={0.0};
...
//for each kinematicas, find the min/max xbj among all targets; 
   ...
   for(int ii=0;ii<11;ii++){
       if(ii<10)outfile<<x_min[ii]<<",";
       if(ii==10)outfile<<x_min[ii]<<"};"<<endl;
   }

   outfile<<"xmax[11]={";
   outfile<<fixed<<setprecision(3);
   for(int ii=0;ii<11;ii++){
       if(ii<10)outfile<<x_max[ii]<<",";
       if(ii==10)outfile<<x_max[ii]<<"};"<<endl;
   }

   outfile<<"nBin[11]={";
   for(int ii=0;ii<11;ii++){
       if(ii<10)outfile<<nBin[ii]<<",";
       if(ii==10)outfile<<nBin[ii]<<"};"<<endl;
   }
   outfile.close();
