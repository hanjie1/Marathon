int GetRunList(vector<Int_t> &runList1,vector<Int_t> &runList2,vector<Int_t> &runList3,const int kin,const TString target)
{
     ...
     if(!gSystem->AccessPathName(rundat)){
        ...
        if(tmp.ReadLine(infile)){
           while(tmp.Tokenize(content,from,","))
           {
              ...
              if(run_number>0){runList1.push_back(run_number);nrun++;}
           }
        }
        infile.close();
     }
	  ...
	  rundat=Form("/w/halla-scifs17exp/triton/Runlist/%s_kin%d_1st.dat",target.Data(),kin);
          ...
          if(!gSystem->AccessPathName(rundat)){
              ...
              if(tmp.ReadLine(infile)){
                ...
                {
                   run_number = atoi(content);
                   if(run_number>0){runList2.push_back(run_number);nrun++;}
                }
              }
              infile.close();
          }  
          ...
          if(!gSystem->AccessPathName(rundat)){
              ...
              if(tmp.ReadLine(infile)){
                ...
                {
                   run_number = atoi(content);
                   if(run_number>0){runList3.push_back(run_number);nrun++;}
                }
              }
              infile.close();
          }
