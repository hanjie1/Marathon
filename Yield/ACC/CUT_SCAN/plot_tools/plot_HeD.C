...
void plot_HeD()
{
     ...
     Double_t He_x2[4][MAXBIN],He_Q22[4][MAXBIN],He_xavg2[4][MAXBIN];
     Double_t D2_Y2[4][MAXBIN],D2_YE2[4][MAXBIN]; 
     Double_t He_Y2[4][MAXBIN],He_YE2[4][MAXBIN]; 
     Double_t HeD2[4][MAXBIN],HeD_E2[4][MAXBIN]; 
   ...
   for(int ii=0;ii<4;ii++){
       ...
       Yfile=Form("newbin/He3_kin%d.txt",kin[ii]);
   ...
   for(int ii=0;ii<4;ii++){
	   ...
	   if(He_x[ii][jj]==0)continue;
	     ...
	     HeD_E[ii][jj]=HeD[ii][jj]*sqrt(pow(He_YE[ii][jj]/He_Y[ii][jj],2)+pow(D2_YE[ii][jj]/D2_Y[ii][jj],2));
           ...
           outfile1<<He_xavg[ii][jj]<<"  "<<HeD[ii][jj]<<"  "<<HeD_E[ii][jj]/HeD[ii][jj]<<endl;
           nn++;
       }
   } 
   ...
   for(int ii=0;ii<4;ii++){
	   ...
	   if(He_x1[ii][jj]==0)continue;
	     ...
	     HeD_E1[ii][jj]=HeD1[ii][jj]*sqrt(pow(He_YE1[ii][jj]/He_Y1[ii][jj],2)+pow(D2_YE1[ii][jj]/D2_Y1[ii][jj],2));
           ...
           outfile1<<He_xavg1[ii][jj]<<"  "<<HeD1[ii][jj]<<"  "<<HeD_E1[ii][jj]/HeD1[ii][jj]<<endl;
           nn1++;
       }
   }
   ...
   for(int ii=0;ii<4;ii++){
       for(int jj=0;jj<MAXBIN;jj++){
	   if(He_x2[ii][jj]==0)continue;
           if(abs(He_xavg2[ii][jj]-D2_xavg2[ii][jj])>0.001){cout<<"3 point mismatch !!!"<<He_x2[ii][jj]<<endl; continue;}

           if(D2_Y2[ii][jj]>0){
             HeD2[ii][jj]=He_Y2[ii][jj]/D2_Y2[ii][jj];
	     HeD_E2[ii][jj]=HeD2[ii][jj]*sqrt(pow(He_YE2[ii][jj]/He_Y2[ii][jj],2)+pow(D2_YE2[ii][jj]/D2_Y2[ii][jj],2));
	   }
           hratio2->SetPoint(nn2,He_xavg2[ii][jj],HeD2[ii][jj]);
           hratio2->SetPointError(nn2,0.0,HeD_E2[ii][jj]);
           outfile1<<He_xavg2[ii][jj]<<"  "<<HeD2[ii][jj]<<"  "<<HeD_E2[ii][jj]/HeD2[ii][jj]<<endl;
           nn2++;
       }
   }
   outfile1.close(); 
